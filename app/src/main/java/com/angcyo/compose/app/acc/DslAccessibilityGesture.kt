...
import com.angcyo.compose.basics.toastQQ
import com.angcyo.compose.basics.unit.L
import com.angcyo.compose.basics.unit.MainExecutor
import com.angcyo.compose.basics.unit._screenHeight
import com.angcyo.compose.basics.unit._screenWidth
import com.angcyo.compose.basics.unit.dpi
import com.angcyo.compose.basics.unit.isMain
...
class DslAccessibilityGesture {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && service != null && builder != null) {
                ...
                return if (isMain()) {
                    ...
                    _isDispatched = service.dispatchGesture(
                        builder.build(), _gestureResultCallback, MainExecutor.handler
                    )
    ...
    fun moveDuration(
        startTime: Long = DEFAULT_GESTURE_START_TIME, duration: Long = DEFAULT_GESTURE_MOVE_DURATION
    ) {
    ...
    /**点击*/
    fun touch(
        point: PointF, startTime: Long = this.startTime, duration: Long = this.clickDuration
    ) {
        touch(Path().apply {
    ...
    fun double(point: PointF) {
        //双击, 需要伴随MOVE事件, 才能生效
        touch(
            point.x, point.y, point.x - _randomInt(), point.y + _randomInt()
        )
...
fun DslAccessibilityGesture.move(
    ...
    return touch(
        fromX.toFloat(), fromY.toFloat(), toX.toFloat(), toY.toFloat(), startTime, duration, result
    )
}
