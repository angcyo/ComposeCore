...
import com.angcyo.compose.basics.annotation.DSL
import com.angcyo.compose.basics.annotation.Initial
import com.angcyo.compose.basics.global.app
import com.angcyo.compose.basics.global.isDebug
import com.angcyo.compose.basics.unit.L
import com.angcyo.compose.basics.unit.Page
import com.angcyo.compose.basics.unit.createInstance
import com.angcyo.compose.basics.unit.each
import com.angcyo.compose.basics.unit.file
import com.angcyo.compose.basics.unit.getWifiIP
import com.angcyo.compose.basics.unit.saveTo
import com.angcyo.compose.core.BuildConfig
import com.angcyo.compose.core.objectbox.RObjectBox.default_package_name
import com.angcyo.compose.core.objectbox.RObjectBox.getBox
import com.angcyo.compose.core.objectbox.RObjectBox.getBoxStore
 ...
 *
 * https://docs.objectbox.io/getting-started#kotlin-android
 */
object RObjectBox {
...
//            }
        ...
        try {
            ...
            storeBuilder.androidContext(context.applicationContext)
                .maxSizeInKByte(4 * 1024 * 1024) //默认1G, 改成4G
            ...
            storeBuilder
                ...
                //.maxReaders()
                .maxSizeInKByte(100 * 1024 * 1024) //默认 1G
                //.maxDataSizeInKByte()
                //.inMemory()
                .baseDirectory(baseDirectory)
                .name(_dbName)
            ...
            try {
                ...
                //换个数据库名字, 重新初始化
                initObjectBox(context, packageName, dbName, debug)
            }

        } catch (e: Exception) {
            e.printStackTrace()
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && e is ReflectiveOperationException) {
                L.e("当前的路径下:$pkg, 未发现[MyObjectBox]类. 可以考虑在当前包名下新建[PlaceholderEntity]类")
            }
        }
    }
...
inline fun <reified T : Any> KClass<T>.countBy(
    ...
    val cls = this.java
    return boxOf(cls, packageName).query().apply(block).build().count()
}
...
fun <T> Box<T>.findAll(block: QueryBuilder<T>.() -> Unit = {}): List<T> {
    return query().apply(block).build().find()
}
...
fun <T> Box<T>.findFirst(block: QueryBuilder<T>.() -> Unit = {}): T? {
    return query().apply(block).build().findFirst()
}
...
fun <T> Box<T>.findLastList(limit: Long = 1, block: QueryBuilder<T>.() -> Unit = {}): List<T> {
    val count = query().apply(block).build().count()
    if (count <= 0) {
        //Invalid offset (-1): must be zero or positive
        return emptyList()
    }
    return query().apply(block).build().find(max(count - limit, 0), limit)
}
...
inline fun <reified T : Any> KClass<T>.queryOrCreateEntity(
    ...
    val cls = this.java
    val box = boxOf(cls, packageName)
    val find = box.query().apply(block).build().findFirst()
    if (find == null) {
        val entity = cls.createInstance()
        entity.init()
        box.put(entity)
...
inline fun <reified T : Any> KClass<T>.ensureEntity(
    ...
    val cls = this.java
    val box = boxOf(cls, packageName)
    val find = box.query().apply(query).build().findFirst()
    if (find == null) {
        val entity = cls.createInstance()
        entity.block()
        box.put(entity)
...
inline fun <reified T : Any> KClass<T>.updateOrCreateEntity(
    ...
    val cls = this.java
    val box = boxOf(cls, packageName)
    val entity = box.query().apply(query).build().findFirst() ?: cls.createInstance()
    entity.update()
    return box.put(entity)
}
