...
object Reflect {
    ...
    fun areContentsTheSame(target1: Any?, target2: Any?): Boolean {
        ...
        if (!target1.javaClass.isAssignableFrom(target2.javaClass) || !target2.javaClass.isAssignableFrom(
                target1.javaClass
            )
        ) {
            return false
        ...
        var result = true
        try {
            val declaredFields1 = target1.javaClass.declaredFields
            val declaredFields2 = target2.javaClass.declaredFields
    ...
    /**通过反射调用指定对象的指定方法*/
    fun invokeMethod(
        obj: Any, methodName: String, vararg args: Any?
    ): Any? {
        try {
            val argTypes = args.map { it?.javaClass }.toTypedArray()
            obj.javaClass.apply {
                val method =
                    getDeclaredMethod(methodName, *argTypes) ?: getMethod(methodName, *argTypes)
    ...
    fun invokeStaticMethod(
        className: String, methodName: String, vararg args: Any?
    ): Any? {
        try {
            val argTypes = args.map { it?.javaClass }.toTypedArray()
            Class.forName(className).apply {
...
/**设置对象的指定成员变量的值*/
fun Any?.setMember(
    cls: Class<*>, member: String, value: Any?
) {
    try {
        val memberField = cls.getDeclaredField(member)
        memberField.isAccessible = true
...
fun Any?.invokeMethod(methodName: String, paramTypes: Array<Class<*>>, vararg params: Any): Any? {
    ...
    return this.invokeMethod(
        clazz, methodName, paramTypes, *params
    )
}
...
fun Any?.invokeMethod(
    cls: Class<*>, methodName: String, paramTypes: Array<Class<*>>, vararg params: Any
): Any? {
    if (this == null || TextUtils.isEmpty(methodName)) {
        return null
    }
